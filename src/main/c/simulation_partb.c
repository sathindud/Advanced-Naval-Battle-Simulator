...
/**
 * Part 2 B And C are not fully Implemented before the dead line. 
 * Hope to complete this assignment in future
*/
...
// bool newBattalianSimulation(InitialConditionsBattalian battalian, int escort_count, InitialConditionsEscort * escort, int * fire_count)
// {
//     EscortShipsLog Elog;
//     float ship_max_range = range(battalian.max_angle, battalian.maxV);
...
//     if (!escortSimulation(escort_count, escorts, battalian.position, &health))
//     {
//         battalianSimulation(battalian, escort_count, escorts);
//         while (battalian.position.x != destination.x || battalian.position.y != destination.y)
//         {
...
//             //  Running the simulation
//             removeAttackedEscots(&escort_count, escorts);
